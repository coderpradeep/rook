# -*- coding: utf-8 -*-
"""A collection of ORM sqlalchemy models for Kato"""
...
from kato import app, db, db_engine_specs, sm, utils
from kato.connectors.connector_registry import ConnectorRegistry
from kato.models.helpers import AuditMixinNullable, ImportMixin, set_perm
from kato.viz import viz_types
install_aliases()
...
class Slice(Model, AuditMixinNullable, ImportMixin):
    ...
    def get_explore_url(self, base_url='/kato/explore', overrides=None):
    ...
    def explore_json_url(self):
        """Defines the url to access the slice"""
        return self.get_explore_url('/kato/explore_json')
...
class Dashboard(Model, AuditMixinNullable, ImportMixin):
    ...
    def url(self):
        if self.json_metadata:
            ...
            if default_filters:
                ...
                return '/kato/dashboard/{}/?preselect_filters={}'.format(
                    self.slug or self.id, filters)
        return '/kato/dashboard/{}/'.format(self.slug or self.id)
    ...
    def import_obj(cls, dashboard_to_import, import_time=None):
        """Imports the dashboard from the object to the database.
         ...
         to import/export dashboards between multiple kato instances.
...
class Database(Model, AuditMixinNullable, ImportMixin):
    ...
    @property
    def sql_url(self):
        return '/kato/sql/{}/'.format(self.id)
...
class Log(Model):
    ...
    """ORM object used to log Kato actions to the database"""
    ...
    def roles_with_datasource(self):
        ...
        for r in pv.role:
            if r.name in self.ROLES_BLACKLIST:
                continue
            url = (
                '/kato/approve?datasource_type={self.datasource_type}&'
                'datasource_id={self.datasource_id}&'
