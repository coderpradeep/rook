...
from rook import app, dataframe, db, results_backend, utils
from rook.db_engine_specs import LimitMethod
from rook.jinja_context import get_template_processor
from rook.models.sql_lab import Query
from rook.sql_parse import RookQuery
from rook.utils import get_celery_app, QueryStatus
...
def execute_sql(
    ...
    def handle_error(msg):
        ...
        msg = 'Error: {}. You can find common rook errors and their \
    ...
    rook_query = RookQuery(query.sql)
    executed_sql = rook_query.stripped()
    if not rook_query.is_select() and not database.allow_dml:
        return handle_error(
            'Only `SELECT` statements are allowed against this database')
    if query.select_as_cta:
        if not rook_query.is_select():
            return handle_error(
        ...
        executed_sql = rook_query.as_create_table(query.tmp_table_name)
        query.select_as_cta_used = True
    elif (query.limit and rook_query.is_select() and
