...
from rook.connectors.sqla.models import AnnotationDatasource, SqlaTable
from rook.forms import CsvToDatabaseForm
from rook.legacy import cast_form_data
import rook.models.core as models
from rook.models.sql_lab import Query
from rook.sql_parse import RookQuery
from rook.utils import has_access, merge_extra_filters, QueryStatus
from .base import (
    api, BaseRookView, CsvResponse, DeleteMixin,
    generate_download_headers, get_error_msg, get_user_roles,
    json_error_response, RookFilter, RookModelView, YamlExportMixin,
)
...
def check_ownership(obj, raise_if_false=True):
    ...
    security_exception = utils.RookSecurityException(
...
class DashboardFilter(RookFilter):
...
class DatabaseView(RookModelView, DeleteMixin, YamlExportMixin):  # noqa
    ...
    add_template = 'rook/models/database/add.html'
    edit_template = 'rook/models/database/edit.html'
...
if config.get('ENABLE_ACCESS_REQUEST'):
    class AccessRequestsModelView(RookModelView, DeleteMixin):
...
class SliceModelView(RookModelView, DeleteMixin):  # noqa
...
class DashboardModelView(RookModelView, DeleteMixin):  # noqa
...
class LogModelView(RookModelView):
...
class Rook(BaseRookView):
    """The base views for Rook!"""
    ...
    def request_access(self):
        ...
        if has_access:
            return redirect('/rook/dashboard/{}'.format(dashboard_id))
    ...
    def slice(self, slice_id):
        ...
        endpoint = '/rook/explore/?form_data={}'.format(
    ...
    def import_dashboards(self):
        ...
        return self.render_template('rook/import_dashboards.html')
    ...
    def testconn(self):
        ...
        try:
            ...
            if database and uri:
                ...
                logging.info('Rook.testconn(). Masked URL: {0}'.format(masked_url))
    ...
    def csrf_token(self):
        return Response(
            self.render_template('rook/csrf_token.json'),
            mimetype='text/json',
        )
    ...
    def fave_dashboards(self, user_id):
        ...
        for o in qry.all():
            ...
            if o.Dashboard.created_by:
                ...
                d['creator'] = str(user)
                d['creator_url'] = '/rook/profile/{}/'.format(
                    user.username)
            payload.append(d)
        return json_success(
            json.dumps(payload, default=utils.json_int_dttm_ser))
    ...
    def dashboard(self, dashboard_id):
        ...
        dash_save_perm = \
            dash_edit_perm and self.can_access('can_save_dash', 'Rook')
    ...
    @expose('/theme/')
    def theme(self):
        return self.render_template('rook/theme.html')
...
appbuilder.add_view_no_menu(Rook)


class CssTemplateModelView(RookModelView, DeleteMixin):
...
def panoramix(url):  # noqa
    return redirect(request.full_path.replace('panoramix', 'rook'))


@app.route('/<regex("caravel\/.*"):url>')
def caravel(url):  # noqa
    return redirect(request.full_path.replace('caravel', 'rook'))
