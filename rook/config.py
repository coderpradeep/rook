# -*- coding: utf-8 -*-
"""The main config file for Rook

All configuration in this file can be overridden by providing a rook_config
in your PYTHONPATH as there is a ``from rook_config import *``
...
from rook.stats_logger import DummyStatsLogger
...
if 'SUPERSET_HOME' in os.environ:
    ...
    DATA_DIR = os.path.join(os.path.expanduser('~'), '.rook')

# ---------------------------------------------------------
# Rook specific config
# ---------------------------------------------------------
...
SQLALCHEMY_DATABASE_URI = 'sqlite:///' + os.path.join(DATA_DIR, 'rook.db')
...
APP_ICON = '/static/assets/images/rook-logo@2x.png'
...
BABEL_DEFAULT_FOLDER = 'rook/translations'
...
DEFAULT_MODULE_DS_MAP = OrderedDict([
    ('rook.connectors.sqla.models', ['SqlaTable']),
    ('rook.connectors.druid.models', ['DruidDatasource']),
])
...
FILENAME = os.path.join(DATA_DIR, 'rook.log')
ROLLOVER = 'midnight'
INTERVAL = 1
BACKUP_COUNT = 30
...
# static http headers to be served by your Rook server.
...
# from CSV files. For example, 'companyname-rook'
...
# Roles that are controlled by the API / Rook and should not be changes
# by humans.
...
try:
    if CONFIG_PATH_ENV_VAR in os.environ:
        ...
        from rook_config import *  # noqa
        import rook_config
        print('Loaded your LOCAL configuration at [{}]'.format(
            rook_config.__file__))
except ImportError:
    pass
