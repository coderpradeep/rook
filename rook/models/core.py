# -*- coding: utf-8 -*-
"""A collection of ORM sqlalchemy models for Rook"""
...
from rook import app, db, db_engine_specs, sm, utils
from rook.connectors.connector_registry import ConnectorRegistry
from rook.models.helpers import AuditMixinNullable, ImportMixin, set_perm
from rook.viz import viz_types
install_aliases()
...
class Slice(Model, AuditMixinNullable, ImportMixin):
    ...
    def get_explore_url(self, base_url='/rook/explore', overrides=None):
    ...
    def explore_json_url(self):
        """Defines the url to access the slice"""
        return self.get_explore_url('/rook/explore_json')
...
class Dashboard(Model, AuditMixinNullable, ImportMixin):
    ...
    def url(self):
        if self.json_metadata:
            ...
            if default_filters:
                ...
                return '/rook/dashboard/{}/?preselect_filters={}'.format(
                    self.slug or self.id, filters)
        return '/rook/dashboard/{}/'.format(self.slug or self.id)
    ...
    def import_obj(cls, dashboard_to_import, import_time=None):
        """Imports the dashboard from the object to the database.
         ...
         to import/export dashboards between multiple rook instances.
...
class Database(Model, AuditMixinNullable, ImportMixin):
    ...
    @property
    def sql_url(self):
        return '/rook/sql/{}/'.format(self.id)
...
class Log(Model):
    ...
    """ORM object used to log Rook actions to the database"""
    ...
    def roles_with_datasource(self):
        ...
        for r in pv.role:
            if r.name in self.ROLES_BLACKLIST:
                continue
            url = (
                '/rook/approve?datasource_type={self.datasource_type}&'
                'datasource_id={self.datasource_id}&'
