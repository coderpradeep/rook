...
class RookQuery(object):
    ...
    def __process_identifier(self, identifier):
        ...
        if '(' not in '{}'.format(identifier):
            self._table_names.add(RookQuery.__get_full_name(identifier))
            return
    ...
    def as_create_table(self, table_name, overwrite=False):
        ...
        :param rook_query: string, sql query that will be executed
    ...
    def __extract_from_token(self, token):
        ...
        for item in token.tokens:
            ...
            if item.ttype in Keyword:
                if RookQuery.__precedes_table_name(item.value.upper()):
            ...
            if item.ttype in Keyword:
                if RookQuery.__is_result_operation(item.value):
                    table_name_preceding_token = False
                    continue
                # FROM clause is over
                break
            ...
            if isinstance(item, IdentifierList):
                for token in item.tokens:
                    if RookQuery.__is_identifier(token):
