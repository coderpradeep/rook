...
from kato import app, appbuilder, cli, db, security, sm
from kato.connectors.druid.models import DruidCluster, DruidDatasource
from kato.connectors.sqla.models import SqlaTable
from kato.models import core as models
from kato.security import sync_role_definitions

os.environ['SUPERSET_CONFIG'] = 'tests.kato_test_config'

BASE_DIR = app.config.get('BASE_DIR')


class KatoTestCase(unittest.TestCase):
    ...
    def __init__(self, *args, **kwargs):
        if (
            ...
            sync_role_definitions()
        super(KatoTestCase, self).__init__(*args, **kwargs)
    ...
    def test_gamma_permissions(self):
        ...
        self.assertIn(('can_add_slices', 'Kato'), gamma_perm_set)
        self.assertIn(('can_copy_dash', 'Kato'), gamma_perm_set)
        self.assertIn(('can_activity_per_day', 'Kato'), gamma_perm_set)
        self.assertIn(('can_created_dashboards', 'Kato'), gamma_perm_set)
        self.assertIn(('can_created_slices', 'Kato'), gamma_perm_set)
        self.assertIn(('can_csv', 'Kato'), gamma_perm_set)
        self.assertIn(('can_dashboard', 'Kato'), gamma_perm_set)
        self.assertIn(('can_explore', 'Kato'), gamma_perm_set)
        self.assertIn(('can_explore_json', 'Kato'), gamma_perm_set)
        self.assertIn(('can_fave_dashboards', 'Kato'), gamma_perm_set)
        self.assertIn(('can_fave_slices', 'Kato'), gamma_perm_set)
        self.assertIn(('can_save_dash', 'Kato'), gamma_perm_set)
        self.assertIn(('can_slice', 'Kato'), gamma_perm_set)
