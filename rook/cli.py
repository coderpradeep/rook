...
from rook import app, db, dict_import_export_util, security, utils
...
def runserver(debug, no_reload, address, port, timeout, workers, socket):
    """Starts a Rook web server."""
    debug = debug or config.get('DEBUG')
    if debug:
        print(Fore.BLUE + '-=' * 20)
        print(
            Fore.YELLOW + 'Starting Rook server in ' +
            Fore.RED + 'DEBUG' +
            Fore.YELLOW + ' mode')
        print(Fore.BLUE + '-=' * 20)
...
def version(verbose):
    ...
    print(Fore.BLUE + '-=' * 15)
    print(Fore.YELLOW + 'Rook ' + Fore.CYAN + '{version}'.format(
...
def refresh_druid(datasource, merge):
    ...
    session = db.session()
    from rook.connectors.druid.models import DruidCluster
...
def worker(workers):
    """Starts a Rook worker for async SQL query execution."""
    if workers:
