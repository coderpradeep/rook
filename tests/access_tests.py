...
from kato import app, db, security, sm
from kato.connectors.connector_registry import ConnectorRegistry
from kato.connectors.druid.models import DruidDatasource
...
EXTEND_ROLE_REQUEST = (
    '/kato/approve?datasource_type={}&datasource_id={}&'
...
class RequestAccessTests(KatoTestCase):
    ...
    def test_override_role_permissions_is_admin_only(self):
        ...
        response = self.client.post(
            '/kato/override_role_permissions/',
    ...
    @mock.patch('kato.utils.send_MIME_email')
    def test_approve(self, mock_send_mime):
        if app.config.get('ENABLE_ACCESS_REQUEST'):
            ...
            self.assertEqual([sm.find_user(username='gamma').email,
                             ...
                             call_args[1])
            self.assertEqual(
                '[Kato] Access to the datasource {} was granted'.format(
    ...
    def test_request_access(self):
        if app.config.get('ENABLE_ACCESS_REQUEST'):
            ...
            ACCESS_REQUEST = (
                ...
                'action={}&')
            ROLE_GRANT_LINK = (
                '<a href="/kato/approve?datasource_type={}&datasource_id={}&'
