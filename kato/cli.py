...
from kato import app, db, dict_import_export_util, security, utils
...
def runserver(debug, no_reload, address, port, timeout, workers, socket):
    """Starts a Kato web server."""
    debug = debug or config.get('DEBUG')
    if debug:
        print(Fore.BLUE + '-=' * 20)
        print(
            Fore.YELLOW + 'Starting Kato server in ' +
            Fore.RED + 'DEBUG' +
            Fore.YELLOW + ' mode')
        print(Fore.BLUE + '-=' * 20)
...
def version(verbose):
    ...
    print(Fore.BLUE + '-=' * 15)
    print(Fore.YELLOW + 'Kato ' + Fore.CYAN + '{version}'.format(
...
def refresh_druid(datasource, merge):
    ...
    session = db.session()
    from kato.connectors.druid.models import DruidCluster
...
def worker(workers):
    """Starts a Kato worker for async SQL query execution."""
    if workers:
