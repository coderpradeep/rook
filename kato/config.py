# -*- coding: utf-8 -*-
"""The main config file for Kato

All configuration in this file can be overridden by providing a kato_config
in your PYTHONPATH as there is a ``from kato_config import *``
...
from kato.stats_logger import DummyStatsLogger
...
if 'SUPERSET_HOME' in os.environ:
    ...
    DATA_DIR = os.path.join(os.path.expanduser('~'), '.kato')

# ---------------------------------------------------------
# Kato specific config
# ---------------------------------------------------------
...
SQLALCHEMY_DATABASE_URI = 'sqlite:///' + os.path.join(DATA_DIR, 'kato.db')
...
APP_ICON = '/static/assets/images/kato-logo@2x.png'
...
BABEL_DEFAULT_FOLDER = 'kato/translations'
...
DEFAULT_MODULE_DS_MAP = OrderedDict([
    ('kato.connectors.sqla.models', ['SqlaTable']),
    ('kato.connectors.druid.models', ['DruidDatasource']),
])
...
FILENAME = os.path.join(DATA_DIR, 'kato.log')
ROLLOVER = 'midnight'
INTERVAL = 1
BACKUP_COUNT = 30
...
# static http headers to be served by your Kato server.
...
# from CSV files. For example, 'companyname-kato'
...
# Roles that are controlled by the API / Kato and should not be changes
# by humans.
...
try:
    if CONFIG_PATH_ENV_VAR in os.environ:
        ...
        from kato_config import *  # noqa
        import kato_config
        print('Loaded your LOCAL configuration at [{}]'.format(
            kato_config.__file__))
except ImportError:
    pass
