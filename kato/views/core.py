...
from kato.connectors.connector_registry import ConnectorRegistry
from kato.connectors.sqla.models import AnnotationDatasource, SqlaTable
from kato.exceptions import KatoException, KatoSecurityException
from kato.forms import CsvToDatabaseForm
from kato.legacy import cast_form_data
import kato.models.core as models
from kato.models.sql_lab import Query
from kato.sql_parse import KatoQuery
from kato.utils import (
    has_access, merge_extra_filters, merge_request_params, QueryStatus,
)
from .base import (
    api, BaseKatoView, CsvResponse, DeleteMixin,
    generate_download_headers, get_error_msg, get_user_roles,
    json_error_response, KatoFilter, KatoModelView, YamlExportMixin,
)
...
def check_ownership(obj, raise_if_false=True):
    ...
    security_exception = KatoSecurityException(
...
class DashboardFilter(KatoFilter):
...
class DatabaseView(KatoModelView, DeleteMixin, YamlExportMixin):  # noqa
    ...
    add_template = 'kato/models/database/add.html'
    edit_template = 'kato/models/database/edit.html'
...
if config.get('ENABLE_ACCESS_REQUEST'):
    class AccessRequestsModelView(KatoModelView, DeleteMixin):
...
class SliceModelView(KatoModelView, DeleteMixin):  # noqa
...
class DashboardModelView(KatoModelView, DeleteMixin):  # noqa
...
class LogModelView(KatoModelView):
...
class Kato(BaseKatoView):
    """The base views for Kato!"""
    ...
    def request_access(self):
        ...
        if has_access:
            return redirect('/kato/dashboard/{}'.format(dashboard_id))
    ...
    def slice(self, slice_id):
        ...
        endpoint = '/kato/explore/?form_data={}'.format(
    ...
    def import_dashboards(self):
        ...
        return self.render_template('kato/import_dashboards.html')
    ...
    def testconn(self):
        ...
        try:
            ...
            if database and uri:
                ...
                logging.info('Kato.testconn(). Masked URL: {0}'.format(masked_url))
    ...
    def csrf_token(self):
        return Response(
            self.render_template('kato/csrf_token.json'),
            mimetype='text/json',
        )
    ...
    def fave_dashboards(self, user_id):
        ...
        for o in qry.all():
            ...
            if o.Dashboard.created_by:
                ...
                d['creator'] = str(user)
                d['creator_url'] = '/kato/profile/{}/'.format(
                    user.username)
            payload.append(d)
        return json_success(
            json.dumps(payload, default=utils.json_int_dttm_ser))
    ...
    def dashboard(self, dashboard_id):
        ...
        dash_save_perm = \
            dash_edit_perm and self.can_access('can_save_dash', 'Kato')
    ...
    @expose('/theme/')
    def theme(self):
        return self.render_template('kato/theme.html')
...
appbuilder.add_view_no_menu(Kato)


class CssTemplateModelView(KatoModelView, DeleteMixin):
...
def panoramix(url):  # noqa
    return redirect(request.full_path.replace('panoramix', 'kato'))


@app.route('/<regex("caravel\/.*"):url>')
def caravel(url):  # noqa
    return redirect(request.full_path.replace('caravel', 'kato'))
