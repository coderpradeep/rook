...
from kato import app, dataframe, db, results_backend, sm, utils
from kato.db_engine_specs import LimitMethod
from kato.jinja_context import get_template_processor
from kato.models.sql_lab import Query
from kato.sql_parse import KatoQuery
from kato.utils import get_celery_app, QueryStatus
...
def execute_sql(
    ...
    def handle_error(msg):
        ...
        msg = 'Error: {}. You can find common kato errors and their \
    ...
    kato_query = KatoQuery(query.sql)
    executed_sql = kato_query.stripped()
    if not kato_query.is_select() and not database.allow_dml:
        return handle_error(
            'Only `SELECT` statements are allowed against this database')
    if query.select_as_cta:
        if not kato_query.is_select():
            return handle_error(
        ...
        executed_sql = kato_query.as_create_table(query.tmp_table_name)
        query.select_as_cta_used = True
    elif (query.limit and kato_query.is_select() and
